...
build:
	docker compose $(INFRA) $(MONOLITH) $(GATEWAY) $(NOTIFY) $(CERT) $(ANALYTICS) $(IDENTITY) $(COURSE) $(ENROLLMENT) build

# ── Rebuild + restart a single service ────────────────────────────────────────
# Example: make rebuild SVC=certificate
rebuild:
	docker compose $(INFRA) $(MONOLITH) $(GATEWAY) $(NOTIFY) $(CERT) $(ANALYTICS) $(IDENTITY) $(COURSE) $(ENROLLMENT) up -d --build $(SVC)

# ── Full stack: infra + monolith + all microservices ──────────────────────────
stack-up:
	docker compose $(INFRA) $(MONOLITH) $(GATEWAY) $(NOTIFY) $(CERT) $(ANALYTICS) $(IDENTITY) $(COURSE) $(ENROLLMENT) up -d

stack-down:
	docker compose $(INFRA) $(MONOLITH) $(GATEWAY) $(NOTIFY) $(CERT) $(ANALYTICS) $(IDENTITY) $(COURSE) $(ENROLLMENT) down

# ── Add a microservice alongside the full stack ────────────────────────────────
# Example: make svc-up SVC=notification
svc-up:
	docker compose $(INFRA) $(MONOLITH) $(GATEWAY) $(NOTIFY) $(CERT) $(ANALYTICS) $(IDENTITY) $(COURSE) $(ENROLLMENT) -f services/$(SVC)/docker-compose.yml up -d

svc-down:
	docker compose $(INFRA) $(MONOLITH) $(GATEWAY) $(NOTIFY) $(CERT) $(ANALYTICS) $(IDENTITY) $(COURSE) $(ENROLLMENT) -f services/$(SVC)/docker-compose.yml down

logs:
	docker compose $(INFRA) $(MONOLITH) $(GATEWAY) $(NOTIFY) $(CERT) $(ANALYTICS) $(IDENTITY) $(COURSE) $(ENROLLMENT) logs -f
